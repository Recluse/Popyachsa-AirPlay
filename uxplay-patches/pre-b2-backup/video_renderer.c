...
void video_renderer_size(float *f_width_source, float *f_height_source, float *f_width, float *f_height, int rotation_hint) {
    ...
    logger_log(logger, LOGGER_DEBUG, "begin video stream wxh = %dx%d; source %dx%d (rot=0x%02x)",
    ...
    for (int i = 0; i < n_renderers; i++) {
        ...
        if (hls_video) {
             ...
             * and do native borderless / drag / resize / fullscreen / aspect. */
            {
                ...
                if (hwnd_env && *hwnd_env) {
                    /* Accept both decimal and 0x-prefixed hex. */
                    guintptr hwnd = (guintptr) g_ascii_strtoull(hwnd_env, NULL,
                    ...
                    if (hwnd) {
                        ...
                        if (sink && GST_IS_VIDEO_OVERLAY(sink)) {
                            ...
                            logger_log(logger, LOGGER_INFO,
                                       "PLAN-A overlay: bound videosink %s to HWND 0x%" G_GINTPTR_MODIFIER "x",
                                       sink_name, (guintptr) hwnd);
                        } else {
                            logger_log(logger, LOGGER_ERR,
                                       "PLAN-A overlay: %s element missing or no GstVideoOverlay support",
                                       sink_name);
                        }
                        if (sink) gst_object_unref(sink);
                    } else {
                        logger_log(logger, LOGGER_ERR,
                                   "PLAN-A overlay: UXPLAY_OVERLAY_HWND='%s' parsed to 0", hwnd_env);
                    }
                }
            }
